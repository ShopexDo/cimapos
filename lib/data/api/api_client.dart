...
class ApiClient extends GetxService {
  ...
  ApiClient({@required this.sharedPreferences}) {
    ...
    debugPrint('Token: $token');
    updateHeader(token);
    AppConstants.baseUrlStream.listen((newBaseUrl) {
      // actualice la URL en tiempo de ejecución aquí
      appBaseUrl = newBaseUrl;
    });
  }
  ...
  Future<Response> getDataConfig(String baseUrl, String uri, {Map<String, dynamic> query, Map<String, String> headers}) async {
    try {
      if(Foundation.kDebugMode) {
        print('====> API Call: $appBaseUrl$uri\nToken: $token');
      }
  ...
  Future<Response> postDataLogin(String uri, dynamic body, {Map<String, String> headers}) async {
    try {
      ...
      Http.Response _response = await Http.post(
        Uri.parse(AppConstants.BASE_URL_lOGIN+uri),
        body: jsonEncode(body),
        headers: headers ?? _mainHeaders,
      ).timeout(Duration(seconds: timeoutInSeconds));
      Response response = handleResponse(_response);
